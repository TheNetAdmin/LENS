...
##region == 2**13 --> 8182 --> more than 4096 entries in ait buffe
...
function bench_func() {
					...
					for fence_strategy in "${fence_strategy_array[@]}"; do
						for fence_freq in "${fence_freq_array[@]}"; do
							...
							for stride_size in "${stride_array[@]}"; do
								...
								iter=$((iter + 1))
								progress=$(bc -l <<<"scale=2; 100 * $iter / ${#stride_array[@]}")
								slack_notice $SlackURL "[Progress] \`[$host_name]\` $progress% [$iter / ${#stride_array[@]}]"
							done
	...
	slack_notice $SlackURL "[Finish  ] \`[$host_name]\` <@U01QVMG14HH> check results"
}
