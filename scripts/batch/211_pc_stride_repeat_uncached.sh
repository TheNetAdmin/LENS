...
region_array=(
    ...
    #$((2 ** 10)) $((2 ** 11)) $((2 ** 12)) $((2 ** 13))
    #$((2 ** 14)) $((2 ** 15))
    #$(seq -s ' ' $((2 ** 16)) $((2 ** 16)) $((2 ** 19 - 1))) # [ 64KB, 512KB) per  64KB ->  7
    $(seq -s ' ' $((2 ** 18 + 2 ** 16 + 2 ** 16)) $((2 ** 16)) $((2 ** 19 - 1))) # [ 64KB, 512KB) per  64KB ->  7
)
sub_op_array=(0)
repeat=16
region_align=4096
        ...
        for sub_op in "${sub_op_array[@]}"; do
            for fence_strategy in "${fence_strategy_array[@]}"; do
                for fence_freq in "${fence_freq_array[@]}"; do
                    ...
                    export LENS_MAKE_ARGS="-DCHASING_FENCE_STRATEGY_ID=$fence_strategy -DCHASING_FENCE_FREQ_ID=$fence_freq -DCHASING_FLUSH_AFTER_LOAD=$flush_after_load"
                    ...
                    for region_size in "${region_array[@]}"; do
                        ...
                        iter=$((iter + 1))
                        progress=$(bc -l <<< "scale=2; $iter * 100 / ${#region_array[@]}")
