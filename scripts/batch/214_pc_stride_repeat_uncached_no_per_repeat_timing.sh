...
#     #       8MB
#     $((2 ** 12))
#     $(seq -s ' ' $((2 ** 13)) $((2 ** 13)) $((2 ** 16 - 1))) # [  8KB,  64KB) per   8KB ->  7
#     $(seq -s ' ' $((2 ** 16)) $((2 ** 15)) $((2 ** 18 - 1))) # [ 64KB, 256KB) per  32KB ->  6
#     $(seq -s ' ' $((2 ** 18)) $((2 ** 17)) $((2 ** 20 - 1))) # [256KB,   1MB) per 128KB ->  6
#     $(seq -s ' ' $((2 ** 20)) $((2 ** 19)) $((2 ** 22 - 1))) # [  1MB,   4MB) per 512KB ->  6
#     $(seq -s ' ' $((2 ** 22)) $((2 ** 21)) $((2 ** 24 - 1))) # [  4MB,  16MB) per   2MB ->  6
...
#     $((2 ** 6)) $((2 ** 7)) $((2 ** 8)) $((2 ** 9))
#     $((2 ** 10)) $((2 ** 11))
#     $(seq -s ' ' $((2 ** 12)) $((2 ** 11)) $((2 ** 14 - 1))) # [  4KB,  16KB) per   2KB ->  6
...
prev_region_array=(
    $((2 ** 6)) $((2 ** 7)) $((2 ** 8)) $((2 ** 9))
    $(seq -s ' ' $((2 ** 10)) $((2 ** 8)) $((2 ** 13 - 1)))  # [  1KB,   8KB) per  256B -> 31
            ...
            for sub_op in "${sub_op_array[@]}"; do
                for fence_strategy in "${fence_strategy_array[@]}"; do
                    for fence_freq in "${fence_freq_array[@]}"; do
                        ...
                        export LENS_MAKE_ARGS="-DCHASING_FENCE_STRATEGY_ID=$fence_strategy -DCHASING_FENCE_FREQ_ID=$fence_freq -DCHASING_FLUSH_AFTER_LOAD=$flush_after_load -DCHASING_RECORD_TIMING=$per_repeat_timing"
                        est_total=$(estimate_time_hours ${#stride_array[@]} ${#region_array[@]})
                        est_checkpoint=$(estimate_time_hours ${#stride_array[@]} 1)
                        slack_notice $SlackURL "[Start   ] $(basename "$0") [FenceStrategy=$fence_strategy] [FenceFreq=$fence_freq] [FlushAfterLoad=$flush_after_load] [Repeat=$repeat] [PerRepeatTiming=$per_repeat_timing] [SubOP=$sub_op] [RegionAlign=$region_align] [EstHours=$est_total)] [EstPerCheckpoint=$est_checkpoint]"
                        iter=0
