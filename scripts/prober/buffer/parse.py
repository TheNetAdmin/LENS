...
def get_average_ns(stats, cycle_type, region_size, block_size):
    ...
    res = -1
    for s in stats:
        if (int(s["region_size"]) == region_size) and (
...
def amplification(src_file, out_file):
    ...
    for block_size in block_sizes:
        ...
        r["rmw_buf_read_amp"] = calc_amplification(
            data, "read", 2**14, 2**24, block_size
        )
        r["ait_buf_read_amp"] = calc_amplification(
            data, "read", 2**24, 2**29, block_size
        )
        r["lsq_write_amp"] = calc_amplification(
            data, "write", 2**12, 2**24, block_size
        )
        if block_size == 64:
