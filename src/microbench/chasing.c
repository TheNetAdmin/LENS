...
void chasing_baf_print_help(void)
{
	...
	printk("Available pointer chasing baf benchmarks: \n");
	printk("\tNo.\tName\t\t\tBlock Size (Byte)\n");
	for (i = 0; i < len; i++)
...
int chasing_baf_find_func(uint64_t block_size)
{
	int ret = -1;
	int i;
	int len = sizeof(chasing_baf_func_list) / sizeof(chasing_func_entry_t);

	for (i = 0; i < len; i++) {
		if (chasing_baf_func_list[i].block_size == block_size) {
			ret = i;
			break;
		}
	}
...
int init_chasing_baf_index(uint64_t *cindex, uint64_t csize)
{
	...
	memset(cindex, 0, sizeof(uint64_t) * csize * 2);

	/* TODO: change cindex to an array of struct to simplify the code. */
	for (i = 0; i < csize - 1; i++) {
		do {
			ret = get_rand(&next_pos, csize);
			if (ret != 0)
				return 1;
		} while ((cindex[next_pos * 2 + 0] != 0) || (next_pos == curr_pos));

		/* Forward chasing order */
		cindex[curr_pos * 2 + 0] = next_pos;
		/* Backward chasing order */
		cindex[next_pos * 2 + 1] = curr_pos;
